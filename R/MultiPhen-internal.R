...
##pv heatmap fuction
.heatm<-function(pvs,effPhe, logfun, opts =list(title="",indexMatch = NULL, Colv=TRUE),symbreaks = FALSE, file ="", xlab="phenotypes",ylab="Location"){
  if(!is.list(pvs)) pvs = list(pvs)
  #par(mfrow = c(1,length(pvs)))
  title=opts$mPhen.title#"NFBC"#opts$mPhen.title
  if(symbreaks) col = c(rev(colorRampPalette(brewer.pal(9,"Reds"),bias = 0.5)(250)), colorRampPalette(brewer.pal(9,"Blues"), bias = 0.5)(250))
  else col=colorRampPalette(brewer.pal(9,"Reds"))(250)
...
#   breaks = NULL
  ...
  dendro = Colv
  for(k in 1:length(pvs)){
    main = paste(title,names(pvs)[[k]], if(symbreaks) "beta" else "p value")
    pvm = pvs[[k]]
    if(is.null(pvm)){
      return(NULL)
    }
